...
test('it route create new partner item - POST /api/partner', async () => {
  let body = {
...
test('it route partner item by id - GET /api/partner/:id', async () => {
...
test('it route partner items - GET /api/partner', async () => {
  let endpoint = `${URI}/api/partner`
  let response = await service.get(endpoint)
...
test('it route update partner item by id - PUT /api/partner/:id', async () => {
  let body = {
    name_slugify: 'cromlu3',
    logo: '/something.AAA',
    name: 'cromlu sac',
...
test('it route delete partner item by id - DELETE /api/partner/:id', async () => {
  let UltimateElement = await sql('partner')
  .orderBy('id', 'desc')
  .limit(1)
  .spread(noop)
